...
export default function Text_3D(props) {
  ...
  const config = useMemo(() => ({ font, size: 30 * props.scale, height: 2 * props.scale}), [font])
  useLayoutEffect(() => {
    ref.current.position.x = -137 * props.scale; // 左右
    ref.current.position.y = -150 * props.scale; // 上下
